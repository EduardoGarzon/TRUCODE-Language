...
function executarAnalisadorLexicoAFD(codigo_fonte, tabela_simbolos) {
        ...
        switch (estado) {
            ...
            case 12: // ESTADO 12 (COMENTÁRIO).
                ...
                if (isComentString(caractere)) {
                    ...
                    console.log("TOKEN GERADO ESTADO 13: ", tokens[tokens.length - 1]);
    ...
    if (lexema.length > 0) {
        ...
        if (lexema[0] === '"') {
            ...
            if (lexema[1] === '@') {
                tokens.push(obterToken(lexema, tabela_simbolos));
                console.log("TOKEN GERADO ESTADO 13: ", tokens[tokens.length - 1]); // Token COMENTÁRIO.
...
function obterToken(lexema, tabela_simbolos) {
    ...
    // Numeros Inteiros.
    if (isNumInt(lexema)) {
        return { token: "NUM_INT", lexema };
    }

    // Numeros Reais.
    if (isNumReal(lexema)) {
        return { token: "NUM_REAL", lexema };
    }
