...
const productions = [
    ...
    // DECLARAÇÃO (SÓ DECLARA)
    ["declaracao", ["ID", "(", "tipo", ")", "#"]],

    // ATRIBUIÇÃO (SÓ ATRIBUI, COM EXPRESSÃO)
...
class SLRParser {
    ...
    createActionTable() {
        ...
        return {
            ...
            17: { ')': 'R37' },
            18: {
                '#': 'S106',   // ✅ fecha DECLARAÇÃO imediatamente
                'VALE': 'S20' // continua sendo atribuição
            },
            19: {
            },
            20: {
            ...
            21: {
                '#': 'S106',   // ✅ SHIFT no # (NÃO reduzir expr!)
                ')': 'R38',
                'PASSO': 'R38',
            ...
            22: {
                ...
                // >>>>> PERMITIR OPERADORES DEPOIS DO 1º TERMO <<<<<
                '??': 'S52',
                '##': 'S53',
                'RECUA': 'S54',
                'MATA': 'S55',
            ...
            68: {
                '#': 'S106',   // ✅ SHIFT no # (igual ao estado 21)
                ')': 'R40',
                'PASSO': 'R40',
            ...
            106: {
                ...
                // 🔥🔥🔥 ADICIONE ESTA LINHA:
                '$': 'R2'        // <<< fecha lista_comandos no fim do arquivo
            }
    ...
    createGotoTable() {
        return {
            ...
            12: {
                'tipo': 13,
                'expr': 21      // <-- NOVO (necessário para encadear reduções)
            },
            13: {
                'declaracao': 4,
                'expr': 21      // 🔥 <-- ADICIONE ISTO
            },
            18: {
                'expr': 21,
                'atribuicao': 5,
                'declaracao': 4   // 🔥 CRÍTICO — estava faltando
            },
            20: {
                'valor': 21,
                'expr': 21
            ...
            106: {
                ...
                'comando': 2,
                'lista_comandos': 3,
                'expr': 21        // 🔥 NOVO — evita erro de GOTO
            }
        };
    }
    ...
    reduce(productionNum) {
        ...
        if (this.symbols.length < rhs.length) {
            this.errors.push(
                `ERRO INTERNO: pilha semântica menor que RHS na produção R${productionNum}`
            );
            return; // evita crash
        }
        ...
        try {
            switch (productionNum) {
                ...
                case 11: { // atribuicao -> ID ( tipo ) VALE expr #
                    const nome = rhsSymbols[0].lexema;
                    const tipoDeclarado = rhsSymbols[2].tipo;
                    const tipoValor = rhsSymbols[5].tipo;

                    // 🔥 NOVA REGRA (o que você pediu):
                    // ATRIBUIÇÃO SÓ PODE ACONTECER SE JÁ FOI DECLARADO ANTES

                    if (!tabela_simbolos_semantico.has(nome)) {
                        throw `Erro semântico: variável '${nome}' usada em atribuição sem ter sido declarada antes.`;
                    }

                    const tipoExistente = buscar(nome);

                    if (tipoExistente !== tipoDeclarado) {
                        throw `Erro semântico: tipo na atribuição (${tipoDeclarado}) não bate com a declaração original (${tipoExistente})`;
                    }

                    if (!tiposCompativeis(tipoExistente, tipoValor)) {
                        throw `Erro semântico: tipos incompatíveis em atribuição de '${nome}': ${tipoExistente} <- ${tipoValor}`;
                    }

                    console.log(`ATRIBUIÇÃO VÁLIDA para ${nome}`);
